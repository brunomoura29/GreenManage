<template>
  <div class="w-full">
    ...
    <div class="hidden sm:block overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 shadow-card">
      <table class="w-full text-sm text-left">
        ...
        <thead class="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
          <tr>
            <th class="px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 text-xs">
              Cliente
            </th>
            <th class="px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 text-xs">
              Telefone
            ...
            <th class="px-4 py-3 w-20 text-center font-semibold text-slate-500 dark:text-slate-400 text-xs">
              Ações
            </th>
          </tr>
        </thead>
        ...
        <tbody class="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
          ...
          >
              ...
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center shrink-0">
                  <span class="text-xs font-semibold text-primary-700 dark:text-primary-400">
                    {{ initials(cliente.razao_social || cliente.nome_fantasia) }}
                  </span>
                </div>
                <div class="min-w-0">
                  ...
                  <p v-if="cliente.nome_fantasia && cliente.razao_social" class="text-xs text-slate-400 dark:text-slate-500 truncate">
            ...
            <!-- Email -->
            <td class="px-4 py-3 text-slate-600 dark:text-slate-300 truncate max-w-[200px]">
              {{ cliente.email || '—' }}
            </td>
      ...
      >
        <div class="flex items-start justify-between gap-2">
          ...
          <div class="flex items-center gap-1 shrink-0">
            <button @click="$emit('edit', cliente)" class="p-2 text-slate-400 hover:text-primary-600">
              <Pencil class="w-4 h-4" />
            </button>
            <button @click="$emit('delete', cliente)" class="p-2 text-slate-400 hover:text-red-600">
              <Trash2 class="w-4 h-4" />
            </button>
          </div>
        </div>
    ...
    <!-- Paginação (integrada) -->
    <div v-if="!loading && clientes.length > 0" class="mt-2 text-primary-900/10">
      <BasePagination
...
const paginatedClientes = computed(() => {
  ...
  return props.clientes.slice(start, end)
})
